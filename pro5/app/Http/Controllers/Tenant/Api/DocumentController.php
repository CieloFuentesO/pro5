...
class DocumentController extends Controller
{
    ...
    public function getStatusCpe(Request $request){
        $document = Document::whereExternal_id($request->externail_id)->first();

        if($document){
            return [
                'success' => true,
                'status_id' => $document->state_type_id,
                'status_desc' => $this->getStateTypeDescription($document->state_type_id),
            ];
        }else{
            return [
                'success' => false,
                'message' => "CPE no encontrado."
            ];
        }
    }
