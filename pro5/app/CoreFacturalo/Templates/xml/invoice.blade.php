...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
    ...
    @endforeach
    @if($document->total_ivap > 0)
    <cbc:Note languageLocaleID="2007"><![CDATA[Operación sujeta al IVAP]]></cbc:Note>
    @endif
    ...
    <cac:TaxTotal>
        ...
        @if($document->total_ivap > 0)
        <cac:TaxSubtotal>
            <cbc:TaxableAmount currencyID="{{ $document->currency_type_id }}">{{ $document->total_base_ivap }}</cbc:TaxableAmount>
            <cbc:TaxAmount currencyID="{{ $document->currency_type_id }}">{{ $document->total_ivap }}</cbc:TaxAmount>
            <cac:TaxCategory>
                <cac:TaxScheme>
                    <cbc:ID>1016</cbc:ID>
    ...
    <cac:LegalMonetaryTotal>
        @if ($document->total_ivap > 0)
            <cbc:LineExtensionAmount currencyID="{{ $document->currency_type_id }}">{{ $document->total_base_ivap }}</cbc:LineExtensionAmount>
        @else
    ...
    <cac:InvoiceLine>
        ...
        <cac:TaxTotal>
            ...
            @if($row->total_ivap > 0)
                <cac:TaxSubtotal>
                    <cbc:TaxableAmount currencyID="{{ $document->currency_type_id }}">{{ $row->total_base_ivap }}</cbc:TaxableAmount>
                    <cbc:TaxAmount currencyID="{{ $document->currency_type_id }}">{{ $row->total_ivap }}</cbc:TaxAmount>
                    <cac:TaxCategory>
                        <cbc:Percent>{{ $row->percentage_ivap }}</cbc:Percent>
