...
<table class="full-width mt-10 mb-10">
    ...
    @foreach($document->items as $row)
        ...
        <tr>
            ...
            <td class="text-center align-top">
                @if(isset($row->item) && isset($row->item->IdLoteSelected))
                        @inject('itemLotGroup', 'App\Services\ItemLotsGroupService')
                        {{ \Carbon\Carbon::parse($itemLotGroup->getLoteFechaVenc($row->item->IdLoteSelected))->format('d/m/y') }}
                @elseif(isset($row->item) && isset($row->item->date_of_due))
