...
class AccountController extends Controller
{
    ...
    public function download(Request $request)
    {
        ...
        switch ($type) {
            ...
            case 'adsoft':
                ...
                $data = [
                    'records' => $this->getStructureAdsoft($this->getAllDocuments($d_start, $d_end)),
                ];
    ...
    private function getStructureAdsoft($documents)
    {
        ...
        foreach ($documents as $row)
        {
            $document = [
                ...
                'fecven' => isset($row->invoice->date_of_due) ? Carbon::parse($row->invoice->date_of_due)->format('d/m/Y') : "",
    ...
    private function getStructureFoxcont($documents)
    {
        ...
        return $documents->transform(function($row) {
            return [
                'date_of_issue' => $row->date_of_issue->format('d/m/Y'),
                'date_of_due' => isset($row->invoice->date_of_due) ? $row->invoice->date_of_due->format('d/m/Y') : "",
                'document_type_id' => $row->document_type_id,
                'state_type_id' => $row->state_type_id,
                'series' => $row->series,
                'number' => str_pad($row->number, 7, '0', STR_PAD_LEFT),
