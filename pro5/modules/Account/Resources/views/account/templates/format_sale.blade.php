        ...
        <tr>
            ...
            if (
                ...
                $total_charge = 0;
                    ...
                    $total_taxed = $row['total_taxed'] + $row['total_base_ivap'];
                    $total_exonerated = $row['total_exonerated'];
                    $total_ivap = $row['total_ivap'];
            ...
            <td>{{ $row['document_type_id'] != "07" ? $total_taxed : -abs($total_taxed) }}</td>
            <td>{{ $row['document_type_id'] != "07" ? $total_exonerated : -abs($total_exonerated) }}</td>
            <td>{{ $row['document_type_id'] != "07" ? $total_unaffected : -abs($total_unaffected)  }}</td>
            {{-- Aqui deberia ir $total_isc --}}
            <td>{{ $row['document_type_id'] != "07" ? $total_isc : -abs($total_isc) }} </td>
            <td></td>
            <td>{{ $row['document_type_id'] != "07" ? $total_igv : -abs($total_igv) }}</td>
            <td>{{ $row['document_type_id'] != "07" ? $total_plastic_bag_taxes : -abs($total_plastic_bag_taxes) }}</td>
            <td>{{ $row['document_type_id'] != "07" ? $total_ivap+$total_charge : -abs($total_ivap+$total_charge) }}</td>
            <td>{{ $row['document_type_id'] != "07" ? $total : -abs($total) }}</td>
